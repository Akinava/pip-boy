...
static void updateBoundingBox(uint8_t xmin, uint8_t ymin, uint8_t xmax, uint8_t ymax) {
  ...
  // Serial.println("");
  // Serial.print("  Update   ");
  // Serial.print( xUpdateMin); Serial.print(" | ");
  // Serial.print( xUpdateMax); Serial.print(" | ");
  // Serial.print( yUpdateMin); Serial.print(" | ");
  // Serial.print( yUpdateMax); Serial.print(" | ");
...
void ST7565::drawbitmap(uint8_t x, uint8_t y, 
  ...
  for (uint8_t j=0; j<h; j++) {
    for (uint8_t i=0; i<w; i++ ) {
      if (pgm_read_byte(bitmap + i + (j>>3)*w) & _BV(j%8)) {
...
void ST7565::drawstringS(uint8_t x, uint8_t line, String str) {
    for (uint8_t i=0; i<str.length(); i++) {
	drawchar( x+i*6,  line,str.charAt(i) );  //str.charAt(i)
   }
};
...
void ST7565::drawline(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, 
  ...
  if (x0 > x1) { swap(x0, x1); swap(y0, y1); }  // тут завжди з лыва на право
 ...
 //#define LCDHEIGHT 64 
  ...
  for (; x0<=x1; x0++) // тут трохи проблема
  {
		if (steep)  { my_setpixel(y0, x0, color);} 	else { my_setpixel(x0, y0, color) ;}
		err -= dy;
...
void ST7565::my_setpixel(uint8_t x, uint8_t y, uint8_t color) {
  ...
  // x is which column
  if (color) 
    st7565_buffer[x+ (y>>3)*128] |= _BV(7-(y%8));  
  else
    st7565_buffer[x+ (y>>3)*128] &= ~_BV(7-(y%8)); 
...
uint8_t ST7565::getpixel(uint8_t x, uint8_t y) {
  ...
  return (st7565_buffer[x+ (y>>3)*128] >> (7-(y%8))) & 0x1;  
}
...
void ST7565::display(void) {
  ...
  for(p = 0; p < 8; p++) {
    ...
    */
    ...
    maxcol = LCDWIDTH-1;
#endif
   // Serial.print(col); Serial.print(" | "); Serial.print(maxcol); Serial.print(" | "); Serial.println(xUpdateMax);
	...
	//maxcol = 127;
    //Serial.println(st7565_buffer[127]);
	int i = p<<7;
    for(; col <= maxcol; col++) {
	  st7565_data(st7565_buffer[i+col]);
    }
  }
